    ...
    <div class="row">
          ...
          <div class="card-body">
            ...
            <div id="privacy-policy-three" class="privacy-policy">
              <form method="post" action="{{ URL::to('admin/item/update') }}" name="about" id="about"
                          ...
                          <select name="cat_id" class="form-control" id="cat_id"
                            ...
                            @foreach ($getcategory as $category)
                              <option value="{{ $category->id }}"
                                {{ $getitem->cat_id == $category->id ? 'selected' : '' }} data-id="{{ $category->id }}">
                          ...
                          <select name="subcat_id" class="form-control" id="subcat_id">
                            ...
                            @foreach ($getsubcategory as $subcatdata)
                              <option value="{{ $subcatdata->id }}"
                                ...
                                {{ $subcatdata->subcategory_name }}</option>
                            @endforeach
                          </select>
                          @error('subcat_id')
                            <span class="text-danger">{{ $message }}</span>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="getitem_name" name="item_name"
                        placeholder="{{ trans('labels.item_name') }}" value="{{ $getitem->item_name }}">
                      @error('getitem_name')
                        <span class="text-danger">{{ $message }}</span>
                      @enderror
                    </div>
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <select name="addons_id[]" class="form-control selectpicker" multiple data-live-search="true"
                        id="getaddons_id">
                        @foreach ($getaddons as $supplier)
                          <option value="{{ $supplier->id }}"
                            {{ in_array($supplier->id, $selected) ? 'selected' : '' }}>{{ $supplier->name }}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="item_type" class="col-form-label">{{ trans('labels.item_type') }}</label>
                      <div class="col-md-12">
                        <div class="form-check form-check-inline">
                          <input class="form-check-input mr-0" type="radio" name="item_type" id="veg"
                            value="1" @if ($getitem->item_type == 1) checked @endif>
                          <label class="form-check-label" for="veg">{{ trans('labels.veg') }}</label>
                        </div>
                        <div class="form-check form-check-inline">
                          <input class="form-check-input mr-0" type="radio" name="item_type" id="nonveg"
                            value="2" @if ($getitem->item_type == 2) checked @endif>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <div class="col-md-12">
                        <div class="form-check form-check-inline">
                          <input class="form-check-input mr-0" type="radio" onclick="get_variation(this)"
                            name="has_variation" id="yes" value="1"
                            @if ($getitem->has_variation == 1) checked @endif>
                          <label class="form-check-label" for="yes">{{ trans('labels.yes') }}</label>
                        </div>
                        <div class="form-check form-check-inline">
                          <input class="form-check-input mr-0" type="radio" onclick="get_variation(this)"
                            name="has_variation" id="no" value="2"
                            @if ($getitem->has_variation == 2) checked @endif>
                      ...
                      <div class="row">
                        ...
                        <div class="col-3">
                          ...
                          <input type="number" class="form-control" name="quantity" id="quantity"
                            placeholder="quantité" required min="0" value="{{ $getitem->quantity }}">
                          @error('quantity')
                            <span class="text-danger">{{ $message }}</span>
                          @enderror
                        </div>
                ...
                <div class="@if ($getitem->has_variation == 2) dn @endif" id="variations">
                  ...
                  <div class="col-md-12 px-0">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control attribute" name="attribute" id="attribute"
                        placeholder="{{ trans('messages.enter_attribute') }}" value="{{ $getitem->attribute }}">
                      @error('attribute')
                        <span class="text-danger">{{ $message }}</span>
                      @enderror
                    </div>
                    ...
                    <div class="row panel-body" id="del-{{ $variation->id }}">
                      ...
                      <div class="col-sm-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control variation" name="variation[{{ $ky }}]"
                            id="variation" placeholder="{{ trans('labels.variation') }}" required=""
                      ...
                      <div class="col-sm-4 d-none">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control sale_price" id="sale_price"
                            name="sale_price[{{ $ky }}]" placeholder="{{ trans('labels.sale_price') }}"
                            required="" value="{{ $variation->sale_price }}">
                          ...
                          <div class="input-group">
                            <div class="input-group-btn pt-35">
                              <button class="btn btn-danger" type="button"
                                onclick="DeleteVariation('{{ $variation->id }}','{{ $getitem->id }}','{{ URL::to('admin/item/deletevariation') }}')">
                ...
                <button class="btn btn-success btn-add-variations @if ($getitem->has_variation == 2) dn @endif"
                  type="button" onclick="edititem_fields();"> {{ trans('labels.add_varation') }} + </button>
                <div class="row">
                  <div class="col-md-12">
                    <div class="row">
                      <div class="col-md-6">
                        <div class="form-group">
                          <label for="preparation_time"
                            class="col-form-label">{{ trans('labels.preparation_time') }}</label>
                          <input type="text" class="form-control" name="preparation_time" id="preparation_time"
                            value="{{ $getitem->preparation_time }}"
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="tax" id="tax"
                            value="{{ $getitem->tax }}" placeholder="{{ trans('labels.tax') }}">
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div class="col-12">
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AddProduct"
          data-whatever="@addProduct">{{ trans('labels.add_image') }}</button>
        <div id="card-display">
              ...
              <div class="col-lg-2 col-md-4 col-sm-6 my-card dataid{{ $itemimage->id }}" id="table-image">
                ...
                <div class="actioncenter">
                  <a href="javascript:void(0)" class="badge badge-info"
                    onClick="updateItemImage('{{ $itemimage->id }}','{{ URL::to('admin/item/showimage') }}')"
                    title="{{ trans('labels.edit') }}"><i class="fa fa-edit p-2" aria-hidden="true"></i> </a>
                  @if (count($getitemimages) > 1)
                    <a href="javascript:void(0)" class="badge badge-danger"
                      @if (env('Environment') == 'sendbox') type="button" onclick="myFunction()" @else onClick="deleteItemImage('{{ $itemimage->id }}','{{ $itemimage->item_id }}','{{ URL::to('admin/item/destroyimage') }}')" @endif
                      title="{{ trans('labels.delete') }}"><i class="fa fa-trash p-2" aria-hidden="true"></i> </a>
                  @endif
                </div>
              </div>
            @endforeach
      ...
      <form method="post" name="editimg" class="editimg" id="editimg" enctype="multipart/form-data">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button class="btn btna-secondary" type="button"
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <button type="button" class="close" data-dismiss="modal"
              aria-label="{{ trans('labels.close') }}"><span aria-hidden="true">&times;</span></button>
          </div>
          <span id="iiemsg"></span>
          <div class="modal-body">
            <div class="form-group">
