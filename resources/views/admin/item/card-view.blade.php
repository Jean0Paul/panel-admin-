          ...
          <div class="card-body py-3">
            ...
            <div class="item-details px-2">
              ...
              <p class="d-flex justify-content-between my-0">{{ trans('labels.category') }}
                <span>{{ @$item['category_info']->category_name }}</span>
              </p>
              ...
              <p class="d-flex justify-content-between my-0">{{ trans('labels.tax') }}
                <span>{{ number_format($item->tax, 2) }}%</span>
              </p>

              <p class="d-flex justify-content-between my-0">Quantité <span>{{ $item->quantity }}%</span>
              </p>
          ...
          <div class="card-footer py-0">
            ...
            <div class="row justify-content-center">
              ...
              @if ($item->is_featured == 1)
                <a class="btn px-2 text-success d-grid"
                  @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="StatusFeatured('{{ $item->id }}','2','{{ URL::to('admin/item/featured') }}')" @endif><i
                    class="fa fa-check"></i><small>{{ trans('labels.featured') }}</small>

                </a>
              @else
                <a class="btn px-2 text-danger d-grid"
                  @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="StatusFeatured('{{ $item->id }}','1','{{ URL::to('admin/item/featured') }}')" @endif><i
                    class="fa fa-close"></i><small>{{ trans('labels.featured') }}</small>
              ...
              @if ($item->item_status == 1)
                <a class="btn px-2 text-success d-grid"
                  @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="StatusUpdate('{{ $item->id }}','2','{{ URL::to('admin/item/status') }}')" @endif>
              ...
              @else
                <a class="btn px-2 text-danger d-grid"
                  @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="StatusUpdate('{{ $item->id }}','1','{{ URL::to('admin/item/status') }}')" @endif>
              ...
              <a class="btn px-2 text-danger d-grid"
                @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="Delete('{{ $item->id }}','{{ URL::to('admin/item/delete') }}')" @endif>
