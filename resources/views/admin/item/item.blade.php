          ...
          <form action="{{ URL::to('admin/item') }}">
            <div class="input-group mb-3">
              <input type="text" class="form-control rounded" name="search"
                ...
                placeholder="{{ trans('labels.type_and_enter') }}" aria-label="{{ trans('labels.type_and_enter') }}"
                aria-describedby="basic-addon2">
              <div class="input-group-append px-1">
                <select class="form-control rounded" name="option">
                  <option value="" selected>{{ trans('labels.select') }}</option>
                  <option value="veg"
                    @isset($_GET['option']) @if ($_GET['option'] == 'veg') selected @endif @endisset>
                    {{ trans('labels.veg') }}</option>
                  <option value="nonveg"
                    @isset($_GET['option']) @if ($_GET['option'] == 'nonveg') selected @endif @endisset>
                    {{ trans('labels.nonveg') }}</option>
                </select>
              </div>
